...
def process_data(full_path, parameters_to_include, grouping_params, baseline):
    all_data = []
    ...
    baseline_composite = '|||'.join(str(try_float(b)) for b in baseline)

    return result_df, baseline_composite
...
def main(full_path, parameters_to_include, grouping_params, baseline, output_file):
    result_df, _ = process_data(full_path, parameters_to_include, grouping_params, baseline)
