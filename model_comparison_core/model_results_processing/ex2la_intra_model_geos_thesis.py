...
import os
import argparse
from excel2latex import excel_to_latex, convert_latest_excel, excel_to_latex_by_dataset, convert_latest_excel_by_dataset
...
def main():
    ...
    excel_path = r"C:\Users\Jesse\Documents\LaTex\thesis\chapters\Appendix\StAmand2025\figures\data\geos_all_BM_CMU.xlsx"
    output_path = r"C:\Users\Jesse\Documents\LaTex\thesis\chapters\Appendix\StAmand2025\figures\geo_table.tex"
    
    # TABLE CONTENT SETTINGS
    caption = r'GPDMM comparison for top-performing variations of geometries (Geo) and dimensionally reduced features (DR) as initial conditions in the latent space. Dims refers to the number of latent dimensions occupied by the geometry where "NA" is not applicable, "all" is the total number of dimensions in the latent space, and "--" indicates missing data where the Bayesian optimization search paths diverged between datasets (see section \ref{sec:experiments})'  # Table caption
    label = "tab:latent-geometries-app"                   # Label for cross-referencing
    
    # Define parameter columns for tables without a Model column
    parameter_columns = ['Geo', 'Dims', 'DR']  # Set to None to use Model column or auto-detect
    ...
    include_hidden_rows = False   # Whether to include rows that are hidden in Excel
    
    # Whether to split into separate tables by dataset
    split_by_dataset = True      # Set to True to create separate tables for each dataset
    ...
    if args.excel or args.directory:
        ...
        use_split = args.split_by_dataset
        ...
        if args.excel:
            ...
            use_parameter_columns = args.parameter_columns if args.parameter_columns else parameter_columns
                
            # Generate default output file if needed
            output_file = args.output or os.path.splitext(args.excel)[0] + '.tex'
            
            # Convert the file
            if use_split:
                # Create separate tables by dataset
                output_files = excel_to_latex_by_dataset(
                    args.excel, 
                    output_file, 
                    ...
                    parameter_columns=use_parameter_columns
                )
                print(f"Created separate tables by dataset in files: {', '.join(output_files)}")
            else:
            ...
            try:
                ...
                if use_split:
                    # Create separate tables by dataset from most recent Excel file
                    output_files = convert_latest_excel_by_dataset(
                        args.directory, 
                        output_file, 
                    ...
                    # Create a single combined table from most recent Excel file
