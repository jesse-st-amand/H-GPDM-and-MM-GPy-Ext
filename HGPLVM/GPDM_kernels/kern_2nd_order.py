...
class Kern(Parameterized):
    ...
    def __init__(self, input_dim, seq_eps, active_dims, name, useGPU=False, *a, **kw):
        """
        ...
        from GPy.kern.src.psi_comp import PSICOMP_GH
    ...
    def __pow__(self, other):
        ...
        """
        assert np.all(self._all_dims_active == range(
            self.input_dim)), "Can only use kernels, which have their input_dims defined from 0"
        assert np.all(other._all_dims_active == range(
            other.input_dim)), "Can only use kernels, which have their input_dims defined from 0"
    ...
    def _check_input_dim(self, X):
        assert X.shape[
                   1] == self.input_dim, "{} did not specify active_dims and X has wrong shape: X_dim={}, whereas input_dim={}".format(
    ...
    def recombine_Xin(self, X, dL_Xin_partition, dL_Xout_partition):
        dL_dXout = np.zeros([self.N, self.D])
        dL_dX0 = np.zeros([self.N, self.D])
        dL_dXin = dL_Xin_partition
        for i, (x0_i, ep_i) in enumerate(zip(self.seq_x0s, self.seq_eps)):
            dL_dXout[x0_i + 2:ep_i + 1, :] = dL_Xout_partition[x0_i - (2 * i):ep_i - (2 * i) - 1, :]
            dL_dX0[x0_i:x0_i + 2, :] = self.x0_prior.dL_dX(X[x0_i:x0_i + 2, :])
...
class CombinationKernel(Kern):
    ...
    def __init__(self, kernels, name, extra_dims=[], link_parameters=True):
        ...
        seq_eps = reduce(np.union1d,(np.r_[x.seq_eps] for x in kernels))
        # initialize the kernel with the full input_dim
        super(CombinationKernel, self).__init__(input_dim, seq_eps, active_dims, name)
