...
class DynamicsBase:
    ...
    def score(self, Y_test_list, init_t=0, seq_len=10, **kwargs):
        ...
        results = self.arch.data_set_class.get_results(['Y_pred_denorm_list', 'X_preds', 'X_inferences',
                                                        ...
                                                        'Y_test_denorm_list'],
                                                       self.predict_Ys_Xs, Y_test_list, init_t=init_t, seq_len=self.arch.attr_dict['sub_seq_len'],
                                                       **kwargs)
        ...
        scores = self.arch.data_set_class.score(self.arch.attr_dict['sub_seq_len'],self.arch.attr_dict['scoring_method'], true_sequences, pred_sequences,
                                               self.arch.data_set_class.action_IDs_test,
                                               self.arch.data_set_class.results_dict['pred_trajs'])


        '''scores = self.arch.data_set_class.score(self.arch.attr_dict['seq_len'],self.arch.attr_dict['scoring_method'], Y_test_list, results[0],
                                               self.arch.data_set_class.action_IDs_test,
                                               self.arch.data_set_class.results_dict['pred_trajs'])'''
