...
class sphere(geometry_base):
    ...
    def parametric_curve(self, phases, params):
        ...
        return np.hstack([d1,d2,d3])
class spiral(geometry_base):
    def __init__(self,BC, name='spiral'):
        super().__init__(BC, name=name)
...
class fourier(geometry_base):
    ...
    def set_params(self):
        ...
        self.amplitudes = [Param(f'A_{i}', 1.0) for i in range(self.num_waves)]
    ...
    def link_params(self):
        print('WARNING: Fourier params not linked.')
    ...
    def parametric_curve(self, phases, params):
        ...
        frequencies = [1] + params[self.num_waves:2*self.num_waves-1]
        phase_shifts = [0] + params[2*self.num_waves-1:]'''
        amplitudes = params[:self.num_waves]
